...
#define GET_NOTEBOOK_BY_TITLE "SELECT id, title FROM notebook WHERE title=:title"
#define GET_NOTEBOOK_BY_ID  "SELECT id, title FROM notebook WHERE id=:id"
...
#define GET_NOTE "SELECT notebook.title, note.id, note.created_at, note.due_date, note.done_at, note.text FROM note INNER JOIN notebook ON note.notebook=notebook.id WHERE note.id=:id"
// output styles
